...
export default function VisitorsScreen() {
  ...
  const styles = StyleSheet.create({
    ...
    qrDates:   { marginBottom: 16 },
    qrDateText: { color: colors.textMuted, fontSize: 12, textAlign: 'center' },
    closeQrBtn: { backgroundColor: colors.accent, borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
    closeQrText: { color: '#fff', fontSize: 14, fontWeight: '500' },
  });
  ...
  const fetchVisitors = useCallback(async () => {
    try {
      const res = await api.get(API.MY_VISITORS);
  ...
  const handleInvite = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post(API.MY_VISITORS.replace('/me', ''), {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              await api.patch(API.REVOKE_VISITOR(visitor.visitor_id), {});
              fetchVisitors();
            } catch (err) {
